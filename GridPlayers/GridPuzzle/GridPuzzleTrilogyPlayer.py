...
class GridPuzzleTrilogyPlayer(PlaywrightPlayer):
    ...
    async def play(self, solution):
        ...
        for position_index, shape in [(position.r * solution.columns_number + position.c, self._convert_to_shape(value)) for position, value in solution]:
            if await cells[position_index].text_content() == '.':
                continue
