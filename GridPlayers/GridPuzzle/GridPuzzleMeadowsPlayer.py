...
class GridPuzzleMeadowsPlayer(PlaywrightPlayer, GridPuzzleCanvasPlayer):
    game_name = "meadows"
    ...
    async def _draw_regions(self, cell_height, cell_width, page, pairs_positions: list[tuple[Position, Position]], x0, y0):
        for position0, position1 in pairs_positions:
            await self._trace(position0.direction_to(position1), position0, page, cell_width, cell_height, x0, y0)

    @staticmethod
    async def _trace(direction: Direction, position, page, cell_width, cell_height, x0, y0):
