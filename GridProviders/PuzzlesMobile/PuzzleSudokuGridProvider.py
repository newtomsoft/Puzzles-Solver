...
class PuzzleSudokuGridProvider(PlaywrightGridProvider, PuzzlesMobileGridProvider):
    async def scrap_grid(self, browser: BrowserContext, url):
        ...
        numbers_str = [inner_text if (inner_text := await number_div.inner_text()) else SudokuBaseSolver.cell_empty for number_div in numbers_divs]
        cells_count = len(numbers_str)
        side = int(math.sqrt(cells_count))
        conversion_base = side + 1
        numbers = [int(number, conversion_base) if number != SudokuBaseSolver.cell_empty else SudokuBaseSolver.cell_empty for number in numbers_str]
        matrix = []
