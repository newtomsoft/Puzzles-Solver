...
class GridPuzzleShimaguniGridProvider(PlaywrightGridProvider, GridPuzzleTagProvider):
    ...
    def get_grid_from_html(self, html_page: str):
        ...
        for i, cell in enumerate(matrix_cells):
            ...
            # Edges
            if row == 0: opened_grid_matrix[row][col].discard(Direction.up())
            if row == row_count - 1: opened_grid_matrix[row][col].discard(Direction.down())
            if col == 0: opened_grid_matrix[row][col].discard(Direction.left())
            if col == column_count - 1: opened_grid_matrix[row][col].discard(Direction.right())

        opened_grid = Grid(opened_grid_matrix)
        return grid, RegionsGrid.from_opened_grid(opened_grid)
