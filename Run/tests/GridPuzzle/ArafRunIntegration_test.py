...
from Run.Games import ArafConfig

class ArafRunIntegrationTest(unittest.IsolatedAsyncioTestCase):
    async def test_araf_via_run_console(self):
        ArafConfig.register()
        url = "https://gridpuzzle.com/araf/37vej"
        current_dir = os.path.dirname(__file__)
        asset_path = os.path.join(current_dir, "../..", "..", "GridProviders", "GridPuzzle", "tests", "assets", "araf_sample.html")
        with open(asset_path, "r", encoding='utf-8') as f:
            html_content = f.read()

        provider = GridPuzzleArafGridProvider()
        grid = provider.get_grid_from_html(html_content)

        with patch('builtins.input', return_value=url), \
             patch.object(GridPuzzleArafGridProvider, 'get_grid', new_callable=AsyncMock) as mock_get_grid, \
             patch('GridPlayers.GridPuzzle.GridPuzzleArafPlayer.GridPuzzleArafPlayer.play', new_callable=AsyncMock) as mock_play:
