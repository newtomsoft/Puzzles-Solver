...
class ToichikaRunIntegrationTest(unittest.IsolatedAsyncioTestCase):
    async def test_toichika_via_run_console(self):
        url = "https://gridpuzzle.com/toichika/31xrr"

        current_dir = os.path.dirname(__file__)
        asset_path = os.path.join(current_dir, "../..", "..", "GridProviders", "GridPuzzle", "tests", "assets", "toichika_sample.html")
        with open(asset_path, "r", encoding='utf-8') as f:
            html_content = f.read()

        provider = GridPuzzleToichikaGridProvider()
        grid = provider.get_grid_from_html(html_content)

        with patch('builtins.input', return_value=url), \
             patch.object(GridPuzzleToichikaGridProvider, 'get_grid', new_callable=AsyncMock) as mock_get_grid, \
             patch('GridPlayers.GridPuzzle.GridPuzzleToichikaPlayer.GridPuzzleToichikaPlayer.play', new_callable=AsyncMock) as mock_play:
