...
class AkariSolver(GameSolver):
    def __init__(self, data_game: dict[str, any], solver_engine: SolverEngine):
    ...
    def _add_numbers_constraints(self):
        for (current_row, current_column), number in self._number_constraints.items():
            ...
            constraint = self._solver.sum([adjacent_bulb for adjacent_bulb in adjacent_bulbs]) == number
            self._solver.add(constraint)

    def _add_black_cells_constraints(self):
        for (i, j) in self._black_cells:
            self._solver.add(self._solver.Not(self._bulbs_z3[i][j]))
            self._solver.add(self._solver.Not(self._illuminated_z3[i][j]))
            ...
            for c in range(self.columns_number):
                ...
                for c1 in range(c + 1, self.columns_number):
                    if (r, c1) in self._black_cells:
                        break
                    light_constraints.append(self._bulbs_z3[r][c1])
                constraint = self._illuminated_z3[r][c] == self._solver.Or(light_constraints)
                self._solver.add(constraint)
            ...
            for c in range(self.columns_number):
                if (r, c) not in self._black_cells:
                    ...
                    for r1 in range(r - 1, -1, -1):
                        if (r1, c) in self._black_cells:
                            break
                        constraints.append(self._solver.Implies(self._bulbs_z3[r][c], self._solver.Not(self._bulbs_z3[r1][c])))
                    for r1 in range(r + 1, self.rows_number):
                        if (r1, c) in self._black_cells:
                            break
                        constraints.append(self._solver.Implies(self._bulbs_z3[r][c], self._solver.Not(self._bulbs_z3[r1][c])))
                    for c1 in range(c - 1, -1, -1):
                        if (r, c1) in self._black_cells:
                            break
                        constraints.append(self._solver.Implies(self._bulbs_z3[r][c], self._solver.Not(self._bulbs_z3[r][c1])))
                    for c1 in range(c + 1, self.columns_number):
                        if (r, c1) in self._black_cells:
                            break
                        constraints.append(self._solver.Implies(self._bulbs_z3[r][c], self._solver.Not(self._bulbs_z3[r][c1])))
