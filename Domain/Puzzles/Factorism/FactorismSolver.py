...
class FactorismSolver(GameSolver):
    ...
    def _init_model(self):
        ...
        min_header_value = 1
        max_header_value = self.rows_number
        self._row_headers = [self._model.new_int_var(min_header_value, max_header_value, f"row_header_{r}") for r in range(self.rows_number)]
        self._col_headers = [self._model.new_int_var(min_header_value, max_header_value, f"col_header_{c}") for c in range(self.columns_number)]
    ...
    def _add_clues_constraints(self):
        for pos, val in self._clues.items():
            product_var = self._model.new_int_var(1, self.rows_number * self.columns_number, f"product_{pos.r}_{pos.c}")
            self._model.add_multiplication_equality(product_var, [self._row_headers[pos.r], self._col_headers[pos.c]])
            self._model.add(product_var == val)
