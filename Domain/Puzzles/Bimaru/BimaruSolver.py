...
class BimaruSolver(GameSolver):
    ...
    def _add_ship_bottom_implies_constraint(self, position: Position):
        ...
        if position.r > 0:
            is_top_above = self._model.new_bool_var(f"is_top_above_{position.r}_{position.c}")
    ...
    def _add_ship_top_implies_constraint(self, position: Position):
        ...
        if position.r < self.rows_number - 1:
            is_bottom_below = self._model.new_bool_var(f"is_bottom_below_{position.r}_{position.c}")
    ...
    def _add_ship_left_implies_constraint(self, position: Position):
        ...
        if position.c < self.columns_number - 1:
            is_right_right = self._model.new_bool_var(f"is_right_right_{position.r}_{position.c}")
    ...
    def _add_ship_right_implies_constraint(self, position: Position):
        ...
        if position.c > 0:
            is_left_left = self._model.new_bool_var(f"is_left_left_{position.r}_{position.c}")
    ...
    def _add_ship_middle_horizontal_implies_constraint(self, position: Position):
        ...
        if position.c > 0:
            is_left_left = self._model.new_bool_var(f"is_left_left_mh_{position.r}_{position.c}")
        ...
        if position.c < self.columns_number - 1:
            is_right_right = self._model.new_bool_var(f"is_right_right_mh_{position.r}_{position.c}")
    ...
    def _add_ship_middle_vertical_implies_constraint(self, position: Position):
        ...
        if position.r > 0:
            is_top_above = self._model.new_bool_var(f"is_top_above_mv_{position.r}_{position.c}")
        ...
        if position.r < self.rows_number - 1:
            is_bottom_below = self._model.new_bool_var(f"is_bottom_below_mv_{position.r}_{position.c}")
