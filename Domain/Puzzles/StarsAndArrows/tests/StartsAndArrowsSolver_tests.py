...
class StarsAndArrowsSolverTests(TestCase):
    ...
    def test_solution_15x15_evil_0e77w(self):
        """https://gridpuzzle.com/stars-and-arrows/0e77w"""
        grid = Grid([
            ['→', ___, ___, ___, ___, '↙', '↙', '↘', '↘', ___, ___, ___, '↙', ___, '↓'],
        ...
        counts = {
            'left': [__, __, __, __, __, 5, 1, __, 5, __, __, 5, __, 6, 6],
            'up': [__, __, 6, 2, 3, __, __, __, __, 6, 2, 5, __, 9, __]
        }
        expected_solution = Grid([
            [_, _, X, _, _, _, _, _, _, _, _, _, _, _, _],
            [_, X, _, _, _, _, _, _, _, X, _, X, _, X, _],
            [_, X, _, _, _, _, _, X, _, X, _, X, _, X, _],
            [_, X, X, _, _, _, _, X, X, _, _, _, _, X, _],
            [_, _, X, X, X, _, X, _, _, X, _, _, _, _, _],
            [_, _, _, _, _, X, _, _, _, X, X, X, _, _, X],
            [_, _, _, _, _, _, _, X, _, _, _, _, _, _, _],
            [_, _, _, _, _, _, _, _, _, _, _, _, X, _, _],
            [_, X, _, _, _, X, _, _, _, _, X, _, X, X, _],
            [X, _, _, _, _, _, _, X, _, _, _, _, _, X, _],
            [_, _, _, _, _, _, _, _, _, X, _, _, _, X, _],
            [_, _, X, _, X, _, _, _, _, _, _, X, X, X, _],
            [_, _, _, _, _, _, _, _, _, _, _, X, _, _, _],
            [_, _, X, X, X, _, _, X, _, _, _, _, X, X, _],
            [_, X, X, _, _, X, X, _, _, X, _, _, _, X, _]
        ])
        solver = StarsAndArrowsSolver(grid, counts)
        solution = solver.get_solution()
        self.assertEqual(expected_solution, solution)
