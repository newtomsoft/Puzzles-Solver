    ...
    def _add_initials_constraints(self):
        for position, value in self._input_grid:
            if value == self.black_value:
                self._solver.add(self._grid_z3[position] == self.black_value)

    def _add_sums_clues_constraints(self):
        for r, clue in enumerate(self._row_sums_clues):
            if clue != self.empty:
                row_positions = [Position(r, c) for c in range(self.columns_number)]
                self._add_sum_for_line_constraint(row_positions, clue)

        for c, clue in enumerate(self._column_sums_clues):
            if clue != self.empty:
                col_positions = [Position(r, c) for r in range(self.rows_number)]
                self._add_sum_for_line_constraint(col_positions, clue)

    def _add_sum_for_line_constraint(self, line_positions: list, clue: int):
        sum_terms = [If(self._grid_z3[position], self._input_grid[position], 0) for position in line_positions]
