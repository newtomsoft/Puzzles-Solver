...
class MoonsunSolver(GameSolver):
    ...
    def _init_solver(self):
        self._island_bridges_z3 = {island.position: {direction: self._model.new_int_var(0, 1, f"{island.position}_{direction}") for direction in Direction.orthogonal_directions()} for island in
    ...
    def _add_alternating_black_and_withe_constraints(self):
        ...
        for region_id, region in self._regions.items():
            color_region = self._model.new_int_var(1, 2, f"color_region_{region_id}")
        ...
        for region_id, region in self._regions.items():
            ...
            for position in region:
                for neighbors_position in [pos for pos in self._circle_grid.neighbors_positions(position) if pos not in region]:
                    ...
                    self._model.add(colors_regions[neighbor_region_id] != colors_regions[region_id]).only_enforce_if(linked)
    ...
    def _add_same_color_in_region_crossed_constraints(self, region_id, region):
        ...
        is_white_region = self._model.new_bool_var(f"is_white_region_{region_id}")
        is_black_region = self._model.new_bool_var(f"is_black_region_{region_id}")
        ...
        for pos in white_positions:
            ...
            self._model.add(s == 2).only_enforce_if(is_white_region)
            self._model.add(s == 0).only_enforce_if(is_black_region)

        for pos in black_positions:
            s = sum(self._island_bridges_z3[pos][d] for d in Direction.orthogonal_directions())
            self._model.add(s == 2).only_enforce_if(is_black_region)
            self._model.add(s == 0).only_enforce_if(is_white_region)
