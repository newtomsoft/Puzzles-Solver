...
class FubukiSolver(GameSolver):
    ...
    def get_solution(self) -> Grid:
        self._grid_z3 = Grid([[self._model.NewIntVar(1, 9, f"grid_{r}_{c}") for c in range(self._grid.columns_number)] for r in range(self._grid.rows_number)])
        self._add_constraints()
        if self._solver.Solve(self._model) not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
            return Grid.empty()
        self._previous_solution = Grid([[self._solver.Value(self._grid_z3.value(i, j)) for j in range(self.columns_number)] for i in range(self.rows_number)])
    ...
    def _initials_constraints(self):
        for position, value in self._grid:
            if value != -1:
                ...
                self._model.Add(self._grid_z3[position] >= 1)
                self._model.Add(self._grid_z3[position] <= 9)

    def _add_distinct_constraints(self):
        self._model.AddAllDifferent([grid_z3_value for _, grid_z3_value in self._grid_z3])

    def _add_sums_constraints(self):
        for r in range(self.rows_number):
            self._model.Add(sum([self._grid_z3[Position(r, c)] for c in range(self.columns_number)]) == self.row_sums[r])
        for c in range(self.columns_number):
            self._model.Add(sum([self._grid_z3[Position(r, c)] for r in range(self.rows_number)]) == self.column_sums[c])
