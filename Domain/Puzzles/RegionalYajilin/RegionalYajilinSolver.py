...
class RegionalYajilinSolver(GameSolver):
    ...
    def _init_solver(self):
        ...
        for position in [position for position, _ in self._regions_grid if position not in self._island_bridges_z3]:
        ...
        self._black_cells_z3 = {position: Bool(f'p{position}') for position, _ in self._regions_grid if position in self._island_bridges_z3}
    ...
    def _add_black_cell_constraints(self):
        for position, blacks_count in [(position, value) for position, value in self._blacks_count_grid if value >= 0]:
            ...
            self._solver.add(sum([self._black_cells_z3[position] for position in region_positions if position in self._island_bridges_z3.keys()]) == blacks_count)

    def _add_bridges_sum_constraints(self):
        for position in [position for position, value in self._regions_grid]:
