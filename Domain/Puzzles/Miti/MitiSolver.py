...
class MitiSolver(GameSolver):
    ...
    def _ensure_all_islands_grouped(self) -> tuple[IslandGrid, int]:
        ...
        while self._solver.solve(self._model) in (cp_model.OPTIMAL, cp_model.FEASIBLE):
            self._init_island_grid()
            proposition_count += 1
            for position, direction_bridges in self._grid_vars:
    ...
    def _exclude_positions_values_together(self, positions: set[Position]):
        ...
        for position in positions:
            for direction in Direction.orthogonal_directions():
                var = self._grid_vars[position][direction]
                value = self._island_grid[position].direction_position_bridges.get(direction, [0, 0])[1]
                if value == 1:
    ...
    def _add_opposite_constraints(self):
        for position, _ in self._grid_vars:
            if position.up in self._grid_vars:
                self._model.add(self._grid_vars[position][Direction.up()] == self._grid_vars[position.up][Direction.down()])
            if position.down in self._grid_vars:
                self._model.add(self._grid_vars[position][Direction.down()] == self._grid_vars[position.down][Direction.up()])
            if position.left in self._grid_vars:
                self._model.add(self._grid_vars[position][Direction.left()] == self._grid_vars[position.left][Direction.right()])
            if position.right in self._grid_vars:
                self._model.add(self._grid_vars[position][Direction.right()] == self._grid_vars[position.right][Direction.left()])

    def _add_bridges_sum_constraints(self):
        for _, value in self._grid_vars:
    ...
    def _add_dot_constraints(self, dot_position: Position):
        ...
        in_grid_neighbors_positions = [pos for pos in neighbors_positions if pos in self._grid_vars]
        if len(in_grid_neighbors_positions) == 2:
            pos0, pos1 = in_grid_neighbors_positions
            direction = pos0.direction_to(pos1)
            self._model.add(self._grid_vars[pos0][direction] == 0)
            return
    ...
    def _add_not_edge_dots_constraints(self):
        ...
        for position in empty_edge_positions:
            pos0, pos1 = [neighbor for neighbor in position.straddled_neighbors() if neighbor in self._grid_vars]
            direction = pos0.direction_to(pos1)
            self._model.add(self._grid_vars[pos0][direction] == 1)
    ...
    def _add_not_inside_dot_constraints(self, position):
        positions = [neighbor for neighbor in position.straddled_neighbors() if neighbor in self._grid_vars]
    ...
    def _connected_cells_count_var(self, in_grid_neighbors_positions: list[Position]):
        ...
        for pos0, pos1 in paires:
            direction = pos0.direction_to(pos1)
            constraints.append(self._grid_vars[pos0][direction])
