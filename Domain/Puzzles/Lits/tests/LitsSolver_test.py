...
class LitsSolverTest(unittest.TestCase):
    ...
    def test_get_solution_region(self, name, input_grid, expected_solution_region, expected_lits_value):
        ...
        solution_region1 = frozenset([position for position in solution.get_regions().get(expected_lits_value) if
                                      position in tested_region_positions])
    ...
    def test_solution_15x15_hard(self):
        ...
        expected_solution = Grid([
            [1, 1, 1, 4, 4, _, 1, 1, 1, 4, 4, _, _, _, _],
            [1, _, _, _, 4, 4, 3, _, 1, _, 4, 4, 1, 1, _],
            [3, _, 1, 1, 1, _, 3, 3, 4, _, _, 3, _, 1, _],
            [3, 3, 1, _, 4, 4, 3, _, 4, 4, 3, 3, _, 1, _],
            [3, _, 3, 4, 4, _, 1, _, _, 4, _, 3, _, 4, _],
            [2, 3, 3, _, 1, 1, 1, 3, 3, 3, 1, 1, 4, 4, 2],
            [2, _, 3, _, 3, _, 3, _, 3, _, 1, _, 4, _, 2],
            [2, 1, 1, 3, 3, _, 3, 3, 1, _, 1, _, 1, 1, 2],
            [2, _, 1, _, 3, 2, 3, _, 1, 3, 3, 3, 1, _, 2],
            [4, _, 1, _, _, 2, _, 1, 1, _, 3, _, 1, _, 3],
            [4, 4, 3, 3, 3, 2, _, 4, _, 2, 2, 2, 2, 3, 3],
            [_, 4, _, 3, _, 2, 4, 4, 1, 1, _, 4, _, _, 3],
            [2, 2, 2, 2, 1, _, 4, _, 1, _, 4, 4, _, 1, 1],
            [1, _, _, _, 1, 1, 1, _, 1, _, 4, _, 1, _, 1],
            [1, 1, 1, _, _, _, 2, 2, 2, 2, 1, 1, 1, _, 1]
        ])
        lits_solver = LitsSolver(input_grid)
        solution = lits_solver.get_solution()
        self.assertEqual(expected_solution, solution)
    ...
    def test_solution_20x20_hard(self):
        ...
        expected_solution = Grid([
            [1, 1, _, _, 2, 2, 2, 2, _, 2, _, 4, _, 4, 4, _, 1, 1, _, _],
            [1, _, _, _, _, _, _, 3, _, 2, _, 4, 4, _, 4, 4, _, 1, _, _],
            [1, _, 1, 1, _, _, 3, 3, _, 2, _, _, 4, 1, 1, _, _, 1, _, 2],
            [2, _, 1, _, 1, 1, _, 3, _, 2, _, _, _, _, 1, _, 4, 4, _, 2],
            [2, _, 1, _, _, 1, _, 1, _, 3, _, _, 4, _, 1, 4, 4, _, _, 2],
            [2, _, 3, 3, 3, 1, _, 1, 3, 3, _, 4, 4, _, 4, _, _, _, 3, 2],
            [2, 1, _, 3, _, _, 1, 1, _, 3, _, 4, _, 4, 4, 1, 1, 3, 3, _],
            [_, 1, 1, 1, _, _, _, 4, 4, _, 1, 1, 1, 4, _, 1, _, _, 3, _],
            [_, 3, _, 4, 4, _, 1, _, 4, 4, 1, _, _, 3, _, 1, 4, 4, _, _],
            [3, 3, 3, _, 4, 4, 1, 1, 1, _, _, _, 3, 3, 3, _, _, 4, 4, _],
            [_, _, 1, 1, 1, _, 2, _, _, _, 1, 1, 1, _, 2, 2, 2, 2, _, _],
            [_, _, _, _, 1, _, 2, _, 1, 1, 3, _, 1, _, _, 1, _, _, _, _],
            [2, _, 3, _, 2, _, 2, _, 1, _, 3, 3, _, 4, _, 1, _, 1, _, 3],
            [2, 3, 3, 3, 2, _, 2, _, 1, _, 3, _, 4, 4, 1, 1, _, 1, 3, 3],
            [2, _, _, _, 2, 4, 4, _, 3, _, 1, 1, 4, _, _, _, 1, 1, _, 3],
            [2, _, 3, _, 2, _, 4, 4, 3, 3, 1, _, _, 4, 3, 3, 3, _, 2, _],
            [_, _, 3, 3, 1, 1, _, _, 3, _, 1, _, 4, 4, _, 3, _, _, 2, _],
            [1, _, 3, _, _, 1, 2, 2, 2, 2, 3, _, 4, _, 1, _, 3, _, 2, _],
            [1, _, _, 4, 4, 1, _, 1, _, _, 3, 3, 1, 1, 1, 3, 3, 3, 2, _],
            [1, 1, 4, 4, _, _, _, 1, 1, 1, 3, _, _, _, _, _, _, _, _, _]
        ])
        lits_solver = LitsSolver(input_grid)
        solution = lits_solver.get_solution()
        self.assertEqual(expected_solution, solution)
    ...
    def test_solution_20x20_hard_path(self):
        input_grid = Grid([
            ...
            [10, 1, 1, 2, 3, 3, 4, 5, 5, 6, 6, 7, 7, 7, 11, 8, 8, 8, 12, 9],
            [10, 10, 2, 2, 2, 3, 4, 4, 4, 13, 6, 7, 11, 11, 11, 8, 8, 12, 12, 9],
            [10, 14, 15, 15, 3, 3, 3, 4, 4, 13, 6, 6, 11, 11, 11, 16, 12, 12, 17, 17],
            [10, 14, 14, 15, 15, 4, 4, 4, 13, 13, 6, 6, 6, 16, 16, 16, 16, 17, 17, 17],
            [10, 14, 14, 15, 4, 4, 18, 4, 19, 19, 6, 20, 16, 16, 16, 21, 16, 21, 21, 21],
            [22, 14, 14, 14, 4, 18, 18, 23, 23, 19, 6, 20, 16, 24, 16, 21, 21, 21, 21, 25],
            [22, 22, 26, 14, 14, 18, 27, 23, 23, 19, 6, 20, 20, 24, 16, 16, 21, 28, 25, 25],
            [22, 22, 26, 26, 27, 27, 27, 23, 23, 19, 20, 20, 24, 24, 16, 16, 21, 28, 28, 25],
            [22, 29, 26, 26, 30, 31, 27, 31, 19, 19, 32, 28, 28, 28, 28, 28, 28, 28, 33, 25],
            [29, 29, 34, 26, 30, 31, 31, 31, 31, 32, 32, 32, 32, 35, 35, 35, 35, 33, 33, 33],
            [34, 29, 34, 34, 30, 30, 30, 31, 36, 36, 36, 32, 37, 37, 38, 38, 35, 33, 39, 39],
            [34, 29, 34, 34, 30, 40, 30, 36, 36, 36, 37, 37, 37, 38, 38, 39, 39, 33, 39, 39],
            [34, 34, 34, 40, 40, 40, 36, 36, 36, 41, 37, 37, 38, 38, 42, 42, 39, 39, 39, 43],
            [34, 44, 34, 34, 34, 40, 40, 36, 36, 41, 37, 45, 38, 38, 38, 42, 39, 43, 39, 43],
            [46, 44, 44, 44, 44, 44, 47, 36, 41, 41, 37, 45, 45, 48, 42, 42, 42, 43, 43, 43],
            [46, 44, 49, 50, 50, 44, 47, 47, 47, 37, 37, 45, 51, 48, 52, 52, 52, 52, 43, 53],
            [46, 46, 49, 49, 50, 50, 50, 47, 54, 54, 37, 51, 51, 48, 48, 48, 55, 52, 53, 53],
            [49, 49, 49, 50, 50, 56, 50, 56, 54, 54, 54, 51, 51, 57, 55, 55, 55, 52, 53, 53],
            [49, 49, 50, 50, 50, 56, 56, 56, 54, 54, 51, 51, 57, 57, 57, 55, 55, 55, 55, 53]
        ])
        expected_solution = Grid([
            [3, 3, 3, 1, _, _, _, _, 4, 4, _, 3, _, 2, 2, 2, 2, 1, 1, 1],
            [_, 3, _, 1, _, 1, _, 4, 4, _, _, 3, 3, _, 4, _, _, _, _, 1],
            [2, _, 1, 1, _, 1, _, 2, _, 1, _, 3, _, 4, 4, _, _, 4, 4, _],
            [2, _, _, 3, _, 1, 1, 2, _, 1, 4, 4, _, 4, _, 1, 4, 4, _, 1],
            ...
            [_, _, 4, 4, _, 4, 4, 1, _, 1, _, 3, _, 1, _, 3, _, _, _, 1],
            [1, _, _, 1, 1, 1, _, _, 1, 1, _, 3, 3, _, 3, 3, 3, _, 1, 1],
