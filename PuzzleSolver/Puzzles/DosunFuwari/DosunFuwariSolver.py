...
class DosunFuwariSolver(GameSolver):
    ...
    def __init__(self, region_grid: Grid):
        ...
        self._grid_var: Grid | None = None
        self._previous_solution: Grid | None = None

    def get_solution(self) -> Grid:
        self._grid_var = Grid([[self._model.new_int_var(0, 2, f"cell_{r}_{c}") for c in range(self.columns_number)] for r in range(self.rows_number)])
    ...
    def _compute_solution(self):
        ...
        if status not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
            return Grid.empty()
        grid = Grid([[self._solver.Value(self._grid_var[Position(i, j)]) for j in range(self.columns_number)] for i in range(self.rows_number)])
        return grid
    ...
    def _add_initials_constraints(self):
        for position, value in self._region_grid:
            if value == self.wall:
                self._model.Add(self._grid_var[position] == self.cell_empty)
                continue
            self._model.Add(self._grid_var[position] >= self.cell_empty)
            self._model.Add(self._grid_var[position] <= self.white)
    ...
    def _add_count_constraints(self):
        for positions in [positions for region_id, positions in self._positions_by_region.items() if region_id != self.wall]:
            ...
            for position in positions:
                ...
                self._model.Add(self._grid_var[position] == self.black).OnlyEnforceIf(b)
                self._model.Add(self._grid_var[position] != self.black).OnlyEnforceIf(b.Not())
            ...
            for position in positions:
                ...
                self._model.Add(self._grid_var[position] == self.white).OnlyEnforceIf(b)
                self._model.Add(self._grid_var[position] != self.white).OnlyEnforceIf(b.Not())
    ...
    def _add_gravity_constraints(self):
        for position in [position for position, value in self._region_grid if value != self.wall]:
            ...
            self._model.Add(self._grid_var[position] == self.black).OnlyEnforceIf(is_black)
            self._model.Add(self._grid_var[position] != self.black).OnlyEnforceIf(is_black.Not())

            no_down = self._grid_var.is_position_in_edge_down(position)
            ...
            if not no_down and not down_wall:
                ...
                self._model.Add(self._grid_var[position.down] == self.black).OnlyEnforceIf(is_down_black)
                self._model.Add(self._grid_var[position.down] != self.black).OnlyEnforceIf(is_down_black.Not())
                self._model.AddImplication(is_black, is_down_black)

            is_white = self._model.new_bool_var(f"is_white_{position.r}_{position.c}")
            self._model.Add(self._grid_var[position] == self.white).OnlyEnforceIf(is_white)
            self._model.Add(self._grid_var[position] != self.white).OnlyEnforceIf(is_white.Not())

            no_up = self._grid_var.is_position_in_edge_up(position)
            ...
            if not no_up and not up_wall:
                ...
                self._model.Add(self._grid_var[position.up] == self.white).OnlyEnforceIf(is_up_white)
                self._model.Add(self._grid_var[position.up] != self.white).OnlyEnforceIf(is_up_white.Not())
                self._model.AddImplication(is_white, is_up_white)

            if not self._grid_var.is_position_in_edge_down(position):
                is_down_white = self._model.new_bool_var(f"is_down_white_{position.r}_{position.c}")
                self._model.Add(self._grid_var[position.down] == self.white).OnlyEnforceIf(is_down_white)
                self._model.Add(self._grid_var[position.down] != self.white).OnlyEnforceIf(is_down_white.Not())
