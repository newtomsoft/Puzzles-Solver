...
class StarsAndArrowsSolver(GameSolver):
    ...
    def _init_solver(self):
        self._grid_z3 = Grid([[self._model.NewBoolVar(f"cell{r}_{c}") for c in range(self.columns_number)] for r in range(self.rows_number)])
    ...
    def get_solution(self) -> Grid:
        ...
        if self._solver.Solve(self._model) not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
            return Grid.empty()
        solution = Grid([[1 if bool(self._solver.Value(self._grid_z3[r][c])) else 0 for c in range(self.columns_number)] for r in range(self.rows_number)])
    ...
    def _add_columns_count_constraints(self):
        for c, count in enumerate(self._counts_by_edge['up']):
            if count == -1:
                continue
            self._model.Add(sum([self._grid_z3[Position(r, c)] for r in range(self.rows_number)]) == count)

    def _add_rows_count_constraints(self):
        for r, count in enumerate(self._counts_by_edge['left']):
            if count == -1:
                continue
            self._model.Add(sum([self._grid_z3[Position(r, c)] for c in range(self.columns_number)]) == count)
    ...
    def _add_no_star_for_following_neither_array_constraints(self, positions):
        self._model.Add(self._grid_z3[positions] == 0)
    ...
    def _add_one_and_only_one_star_for_following_array_positions_constraint(self, positions):
        self._model.Add(sum([self._grid_z3[position] for position in positions]) == 1)
