...
class HidokuSolver(GameSolver):
    ...
    def _init_model(self):
        self._grid_var = Grid(
            [[self._model.new_int_var(1, self._rows_number * self._columns_number, f"grid_{r}_{c}") for c in range(self._columns_number)] for r in range(self._rows_number)]
        )
    ...
    def get_solution(self) -> Grid:
        ...
        if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
            return self._compute_solution(self._solver)
    ...
    def _compute_solution(self, solver) -> Grid:
        self._previous_solution = Grid([[self._solver.value(self._grid_var[Position(r, c)]) for c in range(self._columns_number)] for r in range(self._rows_number)])
    ...
    def _add_distinct_number_constraints(self):
        self._model.add_all_different(list(self._grid_var.values))
