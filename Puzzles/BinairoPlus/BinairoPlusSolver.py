...
class BinairoPlusSolver(GameSolver):
    def __init__(self, grid: Grid, comparison_operators: dict[str, list[tuple[int, int]]], solver_engine: SolverEngine):
        self._grid = grid
    ...
    def _compute_solution(self) -> Grid:
        ...
        solution = [[self._solver.is_true(model.eval(self._matrix_z3[r][c])) for c in range(self.columns_number)] for r in range(self.rows_number)]
        ...
        for r in range(self.rows_number):
            for c in range(self.columns_number):
                if self._grid.value(r, c) == 0:
                    self._solver.add(self._solver.Not(self._matrix_z3[r][c]))
    ...
    def _add_not_same_3_adjacent_constraints(self):
        for r in range(self.rows_number):
            for c in range(self.columns_number - 2):
                ...
                constraint = self._solver.And(constraints)
                not_constraint = self._solver.Not(constraint)
